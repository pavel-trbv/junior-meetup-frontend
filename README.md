...
Для более удобной навигации по нашей витрине необходимо реализовать функционал поиска, 
тем более, что он изначально был в шаблоне приложения. Предпочтительнее использовать динамический поиск – пользователю 
не нужно нажимать отдельную кнопку, результат выдачи подстраивается под его запрос в реальном времени. 
Наше тестовое API позволяет запросить не все товары, а только товары по поисковому запросу, 
для этого передается специальный аргумент `query`:

```
https://mockapi.local/products?query=<поисковой запрос>
```

Также имеется готовый компонент `<Input />`, который лишь добавляет визуальное оформление для стандартного тега `<input />` из HTML.

#### Задача

- добавьте на поле ввода обработчик, который будет сохранять текущий запрос;
- добавьте возможность передавать в `getProducts` аргумент для поискового запроса;
- передайте значение поискового запроса в запрос товаров с API;
- \* перезапрос товаров при вводе каждого символа сильно нагрузит API и негативно скажется на производительности – подумайте, как можно сократить количество запросов;
...
<details>
  ...
  Для начала следует модифицировать функцию `getProducts`, чтобы она принимала поисковую строку и передавала ее в запрос:
  ...
  Теперь нужно добавить логику обратки ввода и передачи строки в запрос. 
  Первым делом создадим новое состояние для поискового запроса в `<App />`, а также обработчик изменения значения инпута 
  и передадим все это в компонент:
  ...
  Теперь нужно завязаться на это состояние через `useEffect` и при изменении вызывать `getProducts`.
  Однако в текущей реализации у нас сильно связаны запросы кэшбэка и товаров, кэшбэк перезапрашивать не нужно –
  он сохраняется в рамках сессии пользователя. Вынесем запрос кэшбэка в отдельный эффект, 
  а чтобы потом использовать его в запросе товаров – сохраним в отдельное состояние:
  ...
  Теперь мы можем безопасно сделать эффект, который будет смотреть на изменения состояний кэшбэка и поиска 
  и перезапрашивать список товаров:
  ...
  Для оптимизации динамического поиска можно воспользоваться дебаунсом – 
  механизмом, который гарантирует, что функция будет вызвана только после того, 
  как пройдет определенное время с момента ее последнего вызова.
  Время ожидания задается разработчиком, для оптимизации поиска принято использовать значения в 250-300 мс.

  Таким образом, мы должны перезапрашивать товары, 
  только если _пользователь не писал ничего в течение 300 мс_.

  Функция дебаунса является функцией высшего порядка: принимает исходную функцию и возвращает новую debounced-функцию.

  Шаблон уже включает в себя функцию `debounce`, способ использования:

  ```javascript
  const myDebouncedFunc = debounce(myFunc, 300);
  ```

  Нам нужно внедрить использование дебаунса на каком-то из этапов нашей логики.
  Это продвинутый уровень, поэтому решения с объяснением не будет, попробуйте справиться сами :)

  Однако вы можете посмотреть готовое решение в ветке `steps/step-3`.
</details>

###  Дальнейшие шаги

Этот шаблон полностью в вашем распоряжении, 
поэтому вы можете самостоятельно доработать функционал в рамках пет-проекта для портфолио.
Ниже предложены основные фичи, которые точно должны быть на витрине товаров современного сервиса.

##### Состояние загрузки

Сейчас у нас никак не обрабатывается состояние загрузки, и во время первоначального запроса товаров мы видим пустой экран,
а при перезапросе на какое-то время застывают старые товары. Обычно в таких случаях добавляется лоадер – индикатор загрузки,
он бывает двух видов:

- спиннер – крутящийся круг или любая другая иконка
- скелетон – плитка из элементов-заглушек нейтрального цвета, которые повторяют силуэт итоговых компонентов

Для наглядности можно увеличить задержку ответа на запрос в `mockFetch`, со 100 мс до, например, 500 мс,
имитируя плохую скорость интернета у пользователя.

##### Обработка ошибок

Сейчас мы используем мок-запросы и тестовый датасет, однако реальный API может не всегда отвечать корректно,
возможны ошибки, которые не зависят от нас, но их нужно обработать, иначе у пользоваля просто молча перестанет работать сервис.
Обычно в таком случае контент страницы меняют на сообщение об ошибке с кнопкой действия:
повторить попытку или вернуться на главную страницу.

##### Пагинация

Сейчас в нашем датасете всего 50 товаров, однако их может быть и 500, и 5000. Такой объем элементов сложно обработать браузеру,
он начнет глючить. В таких случаях добавляют пагинацию – разбиение списка сущностей на страницы с переключателем "вперед" / "назад".

Также существует более сложная пагинация "бесконечная лента" – скрипт следит за скроллом пользователя, 
и когда он приближается к концу страницы, и бесшовно подставляет следующую пачку товаров.
Таким образом, у пользователя создается ощущение бесконечного скролла.